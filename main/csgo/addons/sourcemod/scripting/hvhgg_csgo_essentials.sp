...
public Action Command_RageQuit(int client, int args) 
{
	...
	// public shame
	PrintToChatAll("\x07\x06[nebula]\x07\x01 \x07%s\x01 just rage quit.", name)
...
public Action Command_ResetScore(int client, int args)
{
	...
	{
		PrintToChat(client, "\x07\x06[nebula]\x07\x01 Your score already is 0.")
	...
	// public shame
	PrintToChatAll("\x07\x06[nebula]\x07\x01 Player \x07%s\x01 just reset their score.", name)
	return Plugin_Handled;
}
