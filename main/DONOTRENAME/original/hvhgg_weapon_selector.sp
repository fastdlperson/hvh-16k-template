...
public Action HandleSpawn(Handle timer, any userId)
{
	...
	CPrintToChat(client, "\x03[nebula]\x01 Use \x03!deagle\x01 or \x03!r8\x01 at any time to set your preference.");
	CPrintToChat(client, "\x03[nebula]\x01 Use \x03!p2000\x01 or \x03!usp\x01 at any time to set your preference.");
	CPrintToChat(client, "\x03[nebula]\x01 Use \x03!tec9\x01/\x03!fiveseven\x01 or \x03!cz\x01 at any time to set your preference.");

	if (g_bPrefersR8[client])
		CPrintToChat(client, "\x03[nebula]\x01 Current preference: \x03R8 Revolver");
	else
		CPrintToChat(client, "\x03[nebula]\x01 Current preference: \x03Desert Eagle");
	
	if (g_bPrefersUSP[client])
		CPrintToChat(client, "\x03[nebula]\x01 Current preference: \x03USP-S");
	else
		CPrintToChat(client, "\x03[nebula]\x01 Current preference: \x03P2000");

	if (g_bPrefersCZ[client])
		CPrintToChat(client, "\x03[nebula]\x01 Current preference: \x03CZ75-Auto");
	else
		CPrintToChat(client, "\x03[nebula]\x01 Current preference: \x03Tec-9/Five-Seven");

	if (g_bPrefersM4A1S[client])
		CPrintToChat(client, "\x03[nebula]\x01 Current preference: \x03M4A1-S");
	else
		CPrintToChat(client, "\x03[nebula]\x01 Current preference: \x03M4A4");
...
public Action Command_Handler(const char[] command, int client, int args)
{
	if (args > 1)
	{
		char com[128] = "\x03[nebula]\x01 Usage: !";
		StrCat(com, sizeof(com), command);
		
		CReplyToCommand(client, com);
	...
	char com[128] = "\x03[nebula]\x01 Current preference: \x03";
